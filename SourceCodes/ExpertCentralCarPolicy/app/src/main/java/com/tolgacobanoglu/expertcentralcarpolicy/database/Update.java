package com.tolgacobanoglu.expertcentralcarpolicy.database;
    ...
    //TODO update expertDetails methodu yazılacak
...
public class Update
{
    ...
    public void updateCarAfterExpert(Context context, String cityCode, String letterGroup, String digitGroup,String report)
    {
        database = FirebaseFirestore.getInstance();
        database.collection("LicensePlates").document(cityCode+letterGroup+digitGroup).update("expertReport",report);
        Transaction transaction = new Transaction("New Expert Report", Storage.getDefaults("companyName",context) + " created new report for "+cityCode+letterGroup+digitGroup);
        Create create = new Create();
        create.newTransaction(transaction);
